...
class TestDeepSeekClient(unittest.TestCase):
    ...
    def test_init_with_explicit_args(self):
        ...
        with patch.dict(os.environ, {"DEEPSEEK_API_KEY": "env-key"}):
            client = DeepSeekClient(api_key="arg-key", base_url="https://arg.com")
            self.assertEqual(client.api_key, "arg-key")
            self.assertEqual(client.base_url, "https://arg.com")

    def test_init_defaults(self):
        """无参数时使用默认值"""
        client = DeepSeekClient(api_key="fake-key-for-init")
        self.assertEqual(client.model, "gpt-4o")
        self.assertEqual(client.max_tokens, 4096)

    def test_init_custom_model(self):
        """自定义模型"""
        client = DeepSeekClient(api_key="fake-key", model="gpt-4", max_tokens=2048)
    ...
    def test_complete_success(self, mock_openai_class):
        ...
        client = DeepSeekClient(api_key="test", base_url="https://test.com")
    ...
    def test_complete_max_tokens(self, mock_openai_class):
        ...
        client = DeepSeekClient(api_key="test", max_tokens=2048)
