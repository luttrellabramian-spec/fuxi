...
class GRPCBridgeTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        """启动 gRPC 测试服务器"""
        # 创建临时数据库文件
        cls.warm_db_fd, cls.warm_db_path = tempfile.mkstemp(suffix=".db")
        os.close(cls.warm_db_fd)
        cls.cold_db_fd, cls.cold_db_path = tempfile.mkstemp(suffix=".db")
        os.close(cls.cold_db_fd)
        
        # 创建自定义的 MemoryServiceServicer，使用临时数据库
        class TestMemoryServiceServicer(fuxi_pb2_grpc.MemoryServiceServicer):
            def __init__(self):
                self.hot_memory = HotMemory()
                self.warm_memory = WarmMemory(db_path=cls.warm_db_path)
                self.cold_memory = ColdMemory(db_path=cls.cold_db_path)
        ...
        fuxi_pb2_grpc.add_MemoryServiceServicer_to_server(
            TestMemoryServiceServicer(), cls.server
        )
    ...
    def tearDownClass(cls):
        ...
        # 清理临时文件
        try:
            os.unlink(cls.warm_db_path)
        except Exception:
            pass
        try:
            os.unlink(cls.cold_db_path)
        except Exception:
            pass
