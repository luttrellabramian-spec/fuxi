...
class ColdMemory:
    ...
    def __init__(self, db_path: str = "cold_memory.db", embedding_model: str = "all-MiniLM-L6-v2"):
        ...
        if _HAS_EMBEDDING:
            try:
                ...
                self._embedding_dim = self._model.get_sentence_embedding_dimension() or 384
            except Exception:
    ...
    def insert_summary(
        ...
        embedding_blob = sqlite3.Binary(np.array(embedding, dtype=np.float32).tobytes()) if embedding else None
        conn = self._get_conn()
        try:
            with conn:
                conn.execute(
